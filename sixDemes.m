...
In this case, we examine the case of six populations arranged in a line, with symmetrical migration between populations
...
eqpoints1[s_,c_,h_,m_]:=Module[{eeq1,eeq2,eeq3,eeq4,eeq5,eeq6,sn,sc},(*Gives the equilibrium solutions for s,c,h,m*)
sn=1/2 (1-c)(1-h s);
sc=c(1-s);
...
eeq5=q5==(q^2 (1-s)+2q(1-q)(sn+sc))/(q^2 (1-s)+2q(1-q)(2sn +sc)+(1-q)^2)/.{q->((1-2m)q5+m*q4+m*q6)};
eeq6=q6==(q^2 (1-s)+2q(1-q)(sn+sc))/(q^2 (1-s)+2q(1-q)(2sn +sc)+(1-q)^2)/.{q->((1-m)q6+m*q5)};
sols=NSolve[{eeq1,eeq2,eeq3,eeq4,eeq5,eeq6},{q1,q2,q3,q4,q5,q6},Reals,Method->"Newton", WorkingPrecision->30];

z1=Table[
If[
Length[sols]==0||Length[ss]!=6,"N",
If[ss[[1,2]]<0-10^-5||ss[[2,2]]<0-10^-5||ss[[3,2]]<0-10^-5||ss[[4,2]]<0-10^-5||ss[[5,2]]<0-10^-5 ||ss[[6,2]]<0-10^-5 ||ss[[1,2]]>1.+10^-5||ss[[2,2]]>1.+10^-5||ss[[3,2]]>1.+10^-5 ||ss[[4,2]]>1.+10^-5||ss[[5,2]]>1.+10^-5||ss[[6,2]]>1.+10^-5,"N",{ss[[1,2]],ss[[2,2]],ss[[3,2]],ss[[4,2]],ss[[5,2]],ss[[6,2]]}]
...
eq5[m_,s_,sc_,sn_]:=(q^2 (1-s)+2q(1-q)(sn+sc))/(q^2 (1-s)+2q(1-q)(2sn +sc)+(1-q)^2)/.{q->((1-2*m)q5+m*q4+m*q6)};
eq6[m_,s_,sc_,sn_]:=(q^2 (1-s)+2q(1-q)(sn+sc))/(q^2 (1-s)+2q(1-q)(2sn +sc)+(1-q)^2)/.{q->((1-m)q6+m*q5)};

(*Jacobian matrix*)
J[m_,s_,sc_,sn_] := {
{D[eq1[m,s,sc,sn], q1],D[eq1[m,s,sc,sn], q2],D[eq1[m,s,sc,sn], q3],D[eq1[m,s,sc,sn], q4],D[eq1[m,s,sc,sn], q5],D[eq1[m,s,sc,sn], q6]},
{D[eq2[m,s,sc,sn], q1],D[eq2[m,s,sc,sn], q2],D[eq2[m,s,sc,sn], q3],D[eq2[m,s,sc,sn], q4],D[eq2[m,s,sc,sn], q5],D[eq2[m,s,sc,sn], q6]},
{D[eq3[m,s,sc,sn], q1],D[eq3[m,s,sc,sn], q2],D[eq3[m,s,sc,sn], q3],D[eq3[m,s,sc,sn], q4],D[eq3[m,s,sc,sn], q5],D[eq3[m,s,sc,sn], q6]},
{D[eq4[m,s,sc,sn], q1],D[eq4[m,s,sc,sn], q2],D[eq4[m,s,sc,sn], q3],D[eq4[m,s,sc,sn], q4],D[eq4[m,s,sc,sn], q5],D[eq4[m,s,sc,sn], q6]},
{D[eq5[m,s,sc,sn], q1],D[eq5[m,s,sc,sn], q2],D[eq5[m,s,sc,sn], q3],D[eq5[m,s,sc,sn], q4],D[eq5[m,s,sc,sn], q5],D[eq5[m,s,sc,sn], q6]},
{D[eq6[m,s,sc,sn], q1],D[eq6[m,s,sc,sn], q2],D[eq6[m,s,sc,sn], q3],D[eq6[m,s,sc,sn], q4],D[eq6[m,s,sc,sn], q5],D[eq6[m,s,sc,sn], q6]},
};
...
nna=Select[nn,#[[1]]>0.5&&#[[2]]<0.5&&#[[3]]<0.5&&#[[4]]<0.5&&#[[5]]<0.5&&#[[6]]<0.5&];(*potential DTE for target pop peripheral*)
nnb=Select[nn,#[[1]]<0.5&&#[[2]]>0.5&&#[[3]]<0.5&&#[[4]]<0.5&&#[[5]]<0.5&&#[[6]]<0.5&];(*potential DTE for target pop 2-peripheral*)
nnc=Select[nn,#[[1]]<0.5&&#[[2]]<0.5&&#[[3]]>0.5&&#[[4]]<0.5&&#[[5]]<0.5&&#[[6]]<0.5&];(*potential DTE for target pop central*)

staa=If[nna!={},Table[J[m,s,sc,sn]/.{q1->nna[[i,1]],q2->nna[[i,2]],q3->nna[[i,3]],q4->nna[[i,4]],q5->nna[[i,5]],q6->nna[[i,6]]},{i,Length[nna]}],"None1"];
sta=If[!StringQ[staa],Table[Eigenvalues[SetPrecision[staa[[j]],MachinePrecision],1,Method->"Arnoldi"],{j,Length[staa]}],"None1"];
stbb=If[nnb!={},Table[J[m,s,sc,sn]/.{q1->nnb[[i,1]],q2->nnb[[i,2]],q3->nnb[[i,3]],q4->nnb[[i,4]],q5->nnb[[i,5]],q5->nnb[[i,6]]},{i,Length[nnb]}],"None1"];
stb=If[!StringQ[stbb],Table[Eigenvalues[SetPrecision[stbb[[j]],MachinePrecision],1,Method->"Arnoldi"],{j,Length[stbb]}],"None1"];
stcc=If[nnc!={},Table[J[m,s,sc,sn]/.{q1->nnc[[i,1]],q2->nnc[[i,2]],q3->nnc[[i,3]],q4->nnc[[i,4]],q5->nnc[[i,5]],q5->nnc[[i,6]]},{i,Length[nnc]}],"None1"];
...
Export["sixRes06.mx", res06];
Export["Dropbox/sixRes06.mx", res06];
res07 = Table[{ss,mqstar[ss]}, {ss, 0.6+dss, 0.7, dss}];
Export["sixRes07.mx", res07];
Export["Dropbox/sixRes07.mx", res07];
res08 = Table[{ss,mqstar[ss]}, {ss, 0.7+dss, 0.8, dss}];
Export["sixRes08.mx", res08];
Export["Dropbox/sixRes08.mx", res08];
res09 = Table[{ss,mqstar[ss]}, {ss, 0.8+dss, 0.9, dss}];
Export["sixRes09.mx", res09];
Export["Dropbox/sixRes09.mx", res09];
res10 = Table[{ss,mqstar[ss]}, {ss, 0.9+dss, 1, dss}];
Export["sixRes10.mx", res10];
Export["Dropbox/sixRes10.mx", res10];

(*Import results and produce plot*)
res62 = Import["sixRes06.mx"];
res72 = Import["sixRes07.mx"];
res82 = Import["sixRes08.mx"];
res92 = Import["sixRes09.mx"];
res102 = Import["sixRes10.mx"];
...
UseFrontEnd[Export["sixResPFinal.mx", resp2]];
Export["Dropbox/sixResPFinal.mx", resp2];
UseFrontEnd[Export["sixResTFinal.mx", rest2]];
Export["Dropbox/sixResTFinal.mx", rest2];
UseFrontEnd[Export["sixResCFinal.mx", resc2]];
Export["Dropbox/sixResCFinal.mx", resc2];
...
p1 = ListPlot[{resp2,rest2,resc2}, Joined->True, Frame->{True,True,False,False}, FrameLabel->{"Fitness (s)","Maximum Tolerated Migration (m^*)"},PlotLabel->"Maximum Tolerated Migration In Linear 6-Deme Network ", PlotRange->{{0.5,1},{0,0.155}},PlotStyle->{Blue,Orange,Brown}, PlotLegends->{"Targeting \[Alpha] ","Targeting \[Beta] ","Targeting \[Gamma] "},Epilog -> {{Directive[Blue, Dashed], pLine},{Directive[Orange, Dashed], tLine},{Directive[Brown, Dashed], cLine},Text[pMax, Offset[{0, 10}, pMax]],Text[cMax, Offset[{-27, 10}, cMax]],Text[tMax, Offset[{30, 10}, tMax], Background->White], Inset[g2, {0.64,0.145},{0,0},{.33,.66}]}, AspectRatio->1];
